...
string write_roster_as_json(string roster_filename, const RosterPlayerArray &players_arr)
{
    ...
    if (!rosterfile)
    {
        return format_str("Failed to open roster %s for writing", roster_filename.c_str());
    }
    ...
    for (RosterPlayerConstIterator player = players_arr.begin(); player != players_arr.end(); ++player)
    {
        ...
        roster.push_back(jplayer);
    }

    rosterfile << roster.dump(2) << endl;
    rosterfile.close();
    return "";
}